...
function postcheckGuige() {
    ...
    /* Act on the event */
    var comb_id = parseInt($("#comb .tab-sel").attr('data-loopindex'))-1;
    var prototype = [];
    if( $('section[data-loopIndex]').eq(comb_id).find('.u-format.count_atrr').length > 0 ){
        var dom = $('section[data-loopIndex]').eq(comb_id).find('.u-format.count_atrr');
        $(dom).each(function(){
