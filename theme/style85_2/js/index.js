...
function loadCity(id, callback){
    $.post('/region.php', { 'yn_region_id': id }, function(data, textStatus, xhr) {
        ...
        var postalFirst = $('[name="postal"]').children().first();
        $('[name="postal"]').children().remove();
        $('[name="postal"]').append(postalFirst);
        var data = JSON.parse(data);
        data.map(function(item, index){
...
function loadPostal(id){
    $.post('/region.php?', { 'parent_id': id,'act':'getcode' }, function (data) {
        var first = $('[name="postal"]').children().first();
        $('[name="postal"]').children().remove();
        $('[name="postal"]').append(first);
        var data = JSON.parse(data);
        data.map(function (item, index) {
            $('[name="postal"]').append('<option value="' + item.post_code + '">' + item.post_code + '</option>');
        });
    })
};
loadstate();
